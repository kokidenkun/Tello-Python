...
def test_cv_loop():
    ...
    while run:
        ...
        tello_status = tello.get_stat()
        cv2.putText(now_frame, text="battery: "+str(tello_status["battery"]), org=(20, 50), 
            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 255, 0), thickness=2, lineType=cv2.LINE_4)
        cv2.putText(now_frame, text="height: "+str(tello_status["height"]), org=(20, 70), 
            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 255, 0), thickness=2, lineType=cv2.LINE_4)
        cv2.putText(now_frame, text="speed: "+str(tello_status["speed"]), org=(20, 90), 
            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 255, 0), thickness=2, lineType=cv2.LINE_4)
        cv2.putText(now_frame, text="time: "+str(tello_status["time"]), org=(20, 110), 
            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 255, 0), thickness=2, lineType=cv2.LINE_4)

        if tello.cmd_now not in ["battery?", "height?", "speed?", "time?"]:
            cmd_now = tello.cmd_now
        else:
            cmd_now = "none"

        cv2.putText(now_frame, text="command: "+cmd_now, org=(700, 700), 
